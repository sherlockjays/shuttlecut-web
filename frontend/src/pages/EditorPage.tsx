...
const CANVAS_THEMES: Record<ThemeId, Record<string, string>> = {
  ...
  red:   { header_bg: "#781414", row_bg: "#500000", header_text: "#ffdcdc", name_text: "#ffdc00", score_text: "#ffdc00", border: "#ff6464", divider: "#c85050", row_div: "#c85050" },
...
export default function EditorPage({ projectId, onBack }: { projectId: number; onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    if (videoAspect > containerAspect) {
      contentW = displayW; contentH = displayW / videoAspect
      ox = 0; oy = (displayH - contentH) / 2
    } else {
      contentH = displayH; contentW = displayH * videoAspect
      ox = (displayW - contentW) / 2; oy = 0
    }
    ...
    // 선수 행
    let y0 = y + header_h
    for (const [name, score] of [[data.player1_name, data.player1_score], [data.player2_name, data.player2_score]] as [string, number][]) {
    ...
    ctx.beginPath(); ctx.moveTo(x, y + header_h); ctx.lineTo(x + bw, y + header_h); ctx.stroke()
    ctx.strokeStyle = t.row_div
    ctx.lineWidth = Math.max(1, 3 * sf)
    ctx.beginPath(); ctx.moveTo(x + 1, y + header_h + row_h); ctx.lineTo(x + bw - 1, y + header_h + row_h); ctx.stroke()
  }, [data.scoreboard_scale, data.scoreboard_theme, data.player1_name, data.player2_name,
      data.player1_score, data.player2_score, data.match_date, data.tournament_name,
    ...
    <div className="min-h-screen bg-gray-900 text-white flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col p-4 gap-3">
          ...
          {/* 랠리 타임라인 */}
          {data.rallies.length > 0 && (videoDuration > 0 || data.total_frames > 0) && (() => {
            const totalFrames = data.total_frames > 0 ? data.total_frames : Math.round(videoDuration * data.fps)
            return (
              <div className="relative w-full h-6 bg-gray-700 rounded-lg overflow-hidden" title="랠리 타임라인 - 클릭하면 해당 구간으로 이동">
                {data.rallies.map((r, i) => {
                  const left = (r[0] / totalFrames) * 100
                  const width = Math.max(0.5, ((r[1] - r[0]) / totalFrames) * 100)
                  return (
                    <div
                      key={i}
                      onClick={() => { if (videoRef.current) videoRef.current.currentTime = r[0] / data.fps }}
                      title={`랠리 ${i + 1}: ${r[2]}-${r[3]}`}
                      className="absolute top-0 h-full cursor-pointer hover:brightness-125 transition-all"
                      style={{ left: `${left}%`, width: `${width}%`, backgroundColor: r[4] === 1 ? "#3b82f6" : r[4] === 2 ? "#ef4444" : "#6b7280" }}
                    />
                  )
                })}
              </div>
        ...
        <div className="w-72 bg-gray-800 border-l border-gray-700 flex flex-col p-4 gap-4 overflow-y-auto">
          ...
          <section>
            ...
            <div className="flex items-center gap-1 mb-2">
              ...
              {SIZES.map(sz => (
                <button key={sz.value} onClick={() => update({ scoreboard_scale: sz.value })}
            ...
            <div className="flex items-center gap-1 mb-2">
              ...
              {THEMES.map(th => (
                <button key={th.id} onClick={() => update({ scoreboard_theme: th.id })}
                  title={th.label}
                  ...
                  style={{ backgroundColor: th.bg, color: th.accent, border: `1px solid ${th.accent}` }}>
                  {th.label[0]}
                </button>
              ))}
            </div>
            ...
            <div className="mt-3 space-y-1 max-h-48 overflow-y-auto">
              {data.rallies.map((r, i) => (
                <div key={i}
                  ...
                  style={{ backgroundColor: r[4] === 1 ? "#1e3a5f" : r[4] === 2 ? "#5f1e1e" : "#374151" }}>
                  <span className="text-gray-300 w-10 shrink-0">랠리 {i + 1}</span>
                  <span className={`font-mono font-medium ${r[4] === 1 ? "text-blue-300" : r[4] === 2 ? "text-red-300" : "text-gray-300"}`}>
                    {r[2]}-{r[3]} → {r[4] === 1 ? r[2]+1 : r[2]}-{r[4] === 2 ? r[3]+1 : r[3]}
                  </span>
                  <button onClick={e => { e.stopPropagation(); update({ rallies: data.rallies.filter((_, j) => j !== i) }) }}
                    className="text-gray-500 hover:text-red-400 ml-1 shrink-0">✕</button>
                </div>
              ))}
            </div>
          </section>
