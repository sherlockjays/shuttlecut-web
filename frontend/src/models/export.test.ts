...
describe("STATUS_LABEL", () => {
  ...
  it("processing은 '처리 중'", () => expect(STATUS_LABEL.processing).toBe("처리 중"));
...
describe("STATUS_CLASS", () => {
  ...
  it("done은 초록색 클래스", () => expect(STATUS_CLASS.done).toBe("bg-green-600 text-white"));
  it("error는 빨간색 클래스", () => expect(STATUS_CLASS.error).toBe("bg-red-600 text-white"));
});
